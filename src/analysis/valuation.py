...
def _verdict(gap: float) -> str:
    """괴리율 → 3등급 판정 (로그 ±`VERDICT_LOG_THRESHOLD` · ADR-0042).

    ⚠ 여기 `±0.671`이라고 적혀 있었는데 **상수는 0.897이다.** 0.671은 KR `ev_ebitda`의
    로그 MAE라, 어느 시점에 다른 후보 값이 문서에만 남은 것으로 보인다. 숫자를 손으로
    옮겨 적지 않도록 이름으로 가리킨다.
...
# ── ③ RIM (잔여이익모델 간이형) ──────────────────────────────────────
# 닫힌 형태는 Ohlson(1995, CAR 11 661-687)의 선형정보동학에서 나온다. 초과이익이
# x^a_{t+1} = w·x^a_t로 감쇠하면 V = B + α₁·x^a이고 α₁ = w/(1+r−w)인데, 여기에
# x^a = B·(ROE−r)를 넣은 것이 아래 식이다. w→1에서 α₁ → 1/r이므로 V = B·ROE/r로
# 매끄럽게 이어진다 — **상단은 다른 식이 아니라 같은 식의 극한이다.**
#
# **w는 이 모형에서 우리가 고르는 유일한 숫자이고, 감도를 혼자 먹는다.** B=100·ROE 20%·
# r 10%면 w=0.21에서 102, 0.62에서 113, 0.8에서 127, 1.0에서 200이다. 중심을 0.62→0.8로
# 한 칸 올리면 +13.8인데, r을 8~12%로 4%p 흔들어야 116~110으로 6.3밖에 안 움직인다
# — **w 한 칸이 r 4%p의 두 배다.**
#
# 그래서 **세 점을 전부 문헌에서 가져온다**(ADR-0039). 이 축에서 우리가 지어낸 숫자는 없다.
# 예전 (0.6, 0.8, 1.0)은 셋 중 둘이 판단값이었고, 코드 주석이 근거로 적은 "연구에 따라
# 0.27~0.73"에는 인용이 없었으며 범위도 틀렸다 — 실제 문헌은 0.20까지 내려간다.
#
#   하단 0.21 — Wang(2023, EAR 32(3) 663-691) · Myers(1999, TAR 74(1)).
#     Compustat·I/B/E/S 98,955관측(1980~2014)에서 LID 모수와 자본비용을 **동시추정**한 값이다.
#     **주의: 이 값은 중심에 쓰면 안 된다.** 그들의 LID는 2변수 시스템이고 짝인 γ ≈ 1.018
#     (1보다 크다)이 장기 초과이익을 나른다. ω만 떼어 이 1변수 식에 넣으면 그 채널이
#     사라진다 — Wang 자신이 1변수 Ohlson(1995) 구현은 **중앙 34.8% 과소평가**한다고 잰다.
#     그 편향 방향이 '하단'이라는 역할과는 맞으므로 하단으로만 쓴다.
#   중심 0.62 — Fama & French(2000, JB 73(2) 161-175). 수익성의 평균회귀 속도가 연 38%이고
#     지속으로 뒤집으면 0.62다. **재는 대상이 이 식과 같아서** 중심으로 골랐다 — FF는
#     수익성(이익/장부가)을 재고 우리 x^a = B·(ROE−r)도 ROE의 초과분이다.
#     한계: FF는 초과이익이 '평균 ROE로' 회귀한다고 재는데 이 식은 '0으로' 소멸한다.
#     평균 ROE > r이면 우리 쪽이 **보수적으로** 틀린다. 38%는 비선형을 상수로 누른 값이다.
#   상단 1.00 — Frankel & Lee(1998, JAE 25(3) 283-319). 3년치 예측 뒤의 잔여이익을 영구
#     지속으로 두는 V/P 문헌의 표준 구현이다.
#
# **Dechow·Hutton·Sloan(1999, JAE 26(1-3) 1-34)을 중심 근거로 삼지 않았다** — 널리 0.62로
# 인용되지만 원표를 열지 못했고, 못 연 숫자는 상수로 박지 않는다. 다만 이 논문의 다른
# 결과는 여기서 중요하다: 이들의 LID는 '1년 후 이익을 그냥 자본화한 값'을 설명력에서
# **이기지 못했다.** w를 정교하게 맞춘다고 값이 좋아진다는 보장이 없다는 뜻이라,
# 여기서 w를 표본으로 추정하려는 사람은 이것부터 읽어야 한다.
#
# 실측이 말하는 것(ADR-0039 · KR 7,656행): **w는 종합 판정을 거의 바꾸지 않는다.**
# 1년 종합 IC가 후보 넷 전부 0.120이고 판정 등급이 바뀌는 종목은 1.74%뿐이다. ③ 적정가
# 자체는 62%의 행이 5% 넘게 움직이는데 가중 23.1%를 지나며 사라진다. 5년 지평에서만
# 낮은 w가 단조롭게 낫다(0.148→0.147→0.143→0.134) — 그것이 채택 근거였다(ADR-0029).
#
# **안고 가는 것**: ρ(③, 1/PBR)이 0.954 → 0.988로 오른다. ③을 '절대가치 축'이라 부르는
# 근거가 그만큼 약해진다(ADR-0010의 전제이기도 하다). 다만 축 간 상관은 오히려 내려간다
# — ρ(③,①) 0.740→0.715 · ρ(③,⑤) 0.752→0.740. 신뢰도의 `n_eff`가 보는 값은 이쪽이다.
#
# **되돌리려면 아래 상수 하나만 고치면 된다.** ADR-0039은 판정의 자를 결과를 본 뒤에
# 12개월에서 5년으로 옮겼다 — 그 이해충돌을 그 문서에 적어 뒀으니 읽고 판단할 것.
# `python scripts/check_rim_persistence.py --adjudicate-on 1년`이 최초 판정을 재현한다.
RIM_PERSISTENCE = (0.21, 0.62, 1.00)
RIM_PERSISTENCE_CENTER = 0.62     # `backtest.py::_rim_discount`도 이 값을 읽는다


def rim_value(bps: float, roe: float, r: float, w: float) -> float:
    """지속계수 w에서의 RIM 적정가 한 점. **식이 사는 곳은 여기 하나다.**

    `backtest.py::_rim_discount`가 제 식을 따로 갖고 있다가 지속계수가 0.9로 갈라져 있었다
    (판정은 0.8). ADR-0009가 옛 백테스트를 접은 이유 중 하나가 정확히 그것이었다.
    """
    if w >= 1.0:
        return bps * roe / r                       # α₁ → 1/r 극한
    return bps + bps * (roe - r) * w / (1 + r - w)


def _rim(bps: float | None, roe: float | None, r: float):
    """지속계수 시나리오 `RIM_PERSISTENCE`, 중심 `RIM_PERSISTENCE_CENTER`. 근거는 위 주석.
    ...
    if not bps or bps <= 0 or roe is None or roe <= 0 or r <= 0:
        return None, None
    vals = {w: max(rim_value(bps, roe, r, w), 0.0) for w in RIM_PERSISTENCE}
    mid = vals[RIM_PERSISTENCE_CENTER]
    fair_pbr = (bps and mid / bps) or None
    lo, hi = min(vals.values()), max(vals.values())
    # 범위 문구도 상수에서 만든다 — 손으로 적으면 상수를 고칠 때 조용히 썩는다.
    span = f"{min(RIM_PERSISTENCE):.2f}~{max(RIM_PERSISTENCE):.2f}"
    return FairValue("수익가치(RIM)", lo, mid, hi,
                     note=f"ROE {roe:.1%}, r {r:.1%}, 지속계수 {span}"), fair_pbr
...
def _forward_value(fwd_eps: float | None, peer_fwd_per: float | None,
    ...
    ⚠ **아래 '근거(실증)'는 근거로 인용하면 안 된다 (ADR-0038).** 두 지표가 모두
    이 규칙을 고르는 자로 쓸 수 없는 것이었다:
      · 목표주가 대조 — 이 도구는 **판정을 시장 기대와 독립적으로 유지하려고** 컨센서스를
        판정에서 뺐다(ADR-0006). 그것을 답안지로 쓰면 화면이 적는 *"두 값은 재료가 겹치지
        않습니다"*가 거짓이 된다. 가까움을 우리가 만든 것이 되기 때문이다
      · 가격오차 — 자기 과거 PER 중앙값은 **현재가에 붙는다**(PER이 평균회귀하면
        `자기중앙값 × 현재EPS ≈ 현재가`). 적정가 방법을 그 자로 고르면 "현재가를 가장 잘
        맞히는 것"을 뽑게 되고, 그러면 ④는 늘 "적정"이라고 말한다
    깨끗한 자는 **미래 수익률**뿐인데 시점별 컨센서스가 없어 지금은 못 낸다. 그래서 이
    규칙은 **근거가 있어서가 아니라 바꿀 근거도 없어서** 유지되고 있다. 숫자는 사실이라
    남겨 두되, 이 규칙을 정당화하는 데 쓰지 마라.

    근거(실증 · 위 경고와 함께 읽을 것): 11종목 횡단면 테스트
    (scripts/check_multiple_rules.py)에서 자기 5년 중앙값이 |log(예측/현재가)|
    최소(0.26)였고, 증권사 목표주가의 내재 멀티플과 **중앙값 기준** +2% 이내로 일치했다.
    피어 선행PER 중앙값은 AI 피어에 소형주가 섞이면 체계적으로 과소 추정된다(오차 0.65).
    규칙 선택 자체는 2026-07-28 재실행에서도 유지됐다 — 가격오차 0.286으로 여전히 최소,
    목표가 대비 +3.4%. 2026-08-10 재실행에서도 같은 자로는 여전히 최소다(0.080 · 0.319).
