...
# 최상위 마디 0.25는 **재서 넣은 값이다**(ADR-0040). 그 전에는 `>15%`가 열려 있어서
# ROE가 15%를 넘으면 이익이 세 배가 돼도 적정 배수가 한 칸도 안 움직였다 — 실측에서
# 삼성전자 17.6%→59.1%, SK하이닉스 45.7%→153.8%가 모두 같은 칸이었다(11종목 중 9곳).
# 쪼개는 대가는 없다: leave-one-out 오차가 **미국 PBR에서 0.443→0.426(−3.9%)**로 줄고
# 나머지 일곱 조합은 ±0.2% 안쪽이며, 한국 백테스트 검증 IC는 0.098로 그대로다.
# **PBR만 좋아지는 것이 이론과 맞는다** — `P/B=(ROE−g)/(r−g)`는 ROE에 선형이라 계속
# 오르지만 `P/E`는 `1/(r−g)`로 수렴한다. 그래서 높은 ROE를 한 칸에 묶으면 PBR만 손해다.
# 재현: `python scripts/check_roe_buckets.py KR|US`
ROE_EDGES = [-math.inf, -0.20, -0.05, 0.0, 0.05, 0.10, 0.15, 0.25, math.inf]
ROE_LABELS = ["≤-20%", "-20~-5%", "-5~0%", "0~5%", "5~10%", "10~15%", "15~25%", ">25%"]
...
# 2026-08-10 재측정 — ROE 최상위 칸을 25%에서 쪼갠 뒤의 값이다(ADR-0040).
# 바뀐 것은 사실상 **US pbr 하나**다(0.443 → 0.426). 나머지는 ±0.005 안쪽이고,
# 그 차이는 스냅숏이 달라 생긴 것이지 구간 변경 때문이 아니다.
# 재현: `python scripts/check_warranted.py KR|US` — 붙여넣을 줄을 그대로 찍어 준다.
LEG_MAE = {
