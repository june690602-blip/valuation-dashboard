...
# 2026-08-05 실측(KR 174종목, 시총 5분위 층화). **결과가 예상을 뒤집었다** —
# 설계는 ①②⑤(배수 기반)가 서로 겹치고 ③ RIM이 독립일 것으로 봤는데, 실제로는
# **③ RIM과 ①이 +0.787로 가장 겹치고** ①과 ⑤는 +0.263에 그쳤다. ADR-0015가 "①과 ⑤가
# 같은 적정 PER을 쓴다"고 적은 것은 **AAPL 한 종목 관찰**이었고, 106종목에서는 그만큼
# 겹치지 않는다. 이 저장소가 두 번 적어 둔 "한 종목으로 일반화하지 말 것"이 또 걸렸다.
#
# 미국은 아직 안 쟀다 — 표본 틀(get_kr_listing)이 한국뿐이라 `check_confidence.py`가
# KR만 잰다. 미국 종목은 `effective_axes`가 `capped=False`를 돌려 **상한이 안 걸린다**.
...
# 낮음"으로 못박은 규칙(valuation.py)을 연장한 것이고, **아직 실측 근거가 없는 판단값**이다.
NEFF_LOW, NEFF_MID = 2.0, 3.0
