...
def _render_quick_start() -> None:
    ...
    _open_links(
        ...
        ("위험 프로파일 자가진단이 있는 홈 열기", "home", False),
    )
    st.caption(
...
2. **요약·판정**에서 적정가 범위, 괴리율, 방법 간 편차를 먼저 봅니다. 괴리율 `+`는 모형 적정가가 현재가보다 높고, `−`는 낮다는 뜻입니다.
3. **밸류에이션 → 재무 분석 → 업종 비교**에서 ‘싸 보이는 이유’가 이익·현금흐름·경쟁력으로 뒷받침되는지 확인합니다.
4. **주가차트**에서 기간별 추세와 시장 대비 성과를 봅니다. 과거 상대성과는 향후 수익 예측이 아닙니다.
5. **자본비용 → 기업·뉴스**에서 반대 근거를 찾습니다. 사후검증(백테스트) 탭은 **무기한 보류**했습니다 — 판정과 다른 계산을 검증하고 있었고, 그 탭이 검증하던 ② 역사적 밴드는 그 뒤 판정에서 빠졌습니다. 자기검증은 요약 탭의 가중치 민감도·방법 간 편차·컨센서스 병기가 대신합니다.
"""
    )
    st.info(
        "방법 간 편차는 여러 평가 방법이 얼마나 비슷한 값을 냈는지를 보여줍니다. "
        "데이터 정확도나 수익 가능성을 보증하는 값이 아닙니다 — 예전에 여기 붙어 있던 "
        "'신뢰도' 등급은 뗐습니다(ADR-0043). 편차가 작다고 판정이 더 맞지는 않았습니다.",
        icon="ℹ️",
    )

    with st.expander("8개 분석 화면 지도", expanded=False):
        st.markdown(
            """
| 알고 싶은 것 | 화면 | 먼저 볼 것 |
|---|---|---|
| 이 회사는 무엇을 하나? | 기업·뉴스 | 사업 개요와 최근 이슈 |
| 결론은 무엇인가? | 요약·판정 | 적정가 범위·괴리율·방법 간 편차, 컨센서스 교차검증 |
| 가격 흐름은 어떤가? | 주가차트 | 기간별 추세·지수 대비 성과 |
| 업종·과거 대비 싼가? | 밸류에이션 | 업종·자기 역사 배수의 방향, 시나리오·민감도 |
...
"""
    ...
    st.info(
        "위험 프로파일 자가진단을 먼저 하면 위험회피계수와 교육용 배분 범위를 포트폴리오 화면에서 함께 비교할 수 있습니다.",
        icon="ℹ️",
    )
    _open_links(
        ("포트폴리오 화면 열기", "portfolio", True),
        ("홈에서 위험 프로파일 자가진단 선택", "home", False),
    )
...
"""
    ...
    st.warning(
        "화면의 위험자산 비중 y*는 입력한 시장 가정 아래의 교육용 모형 참고치입니다. "
        "금융회사의 공식 투자자정보확인서나 개인화된 투자권유가 아니며 공식 절차를 대체하지 않습니다.",
        icon="⚠️",
    )
...
def _render_terms() -> None:
    ...
    with left:
        with st.expander("적정가 · 괴리율 · 방법 간 편차"):
            st.markdown(
                "- **적정가**: 여러 모형이 계산한 참고값입니다. 증권사 목표주가나 보장된 가격이 아닙니다.\n"
                "- **괴리율**: 모형 적정가를 현재가와 비교한 비율입니다. 크기만 보지 말고 평가 방법들의 방향이 같은지 함께 봅니다.\n"
                "- **방법 간 편차**: 평가 방법끼리 얼마나 비슷한 값을 냈는지 나타냅니다. "
                "데이터 품질 점수도, 판정이 맞을 확률도 아닙니다(ADR-0043)."
            )
    ...
    with right:
        ...
        with st.expander("변동성 · 샤프비율 · 백테스트"):
            st.markdown(
                ...
                "- **백테스트**: 무기한 보류. 판정과 다른 계산을 검증하고 있어 입구를 닫았습니다(ADR-0009)."
            )
...
<div class="guide-check">
  <strong>□</strong> 사용된 각 방법의 적정가 방향과 방법 간 편차를 함께 확인했다.<br>
  <strong>□</strong> 이익과 현금흐름이 함께 좋아지는지 봤다.<br>
  <strong>□</strong> 업종 대비 싸거나 비싼 이유를 확인했다.<br>
  <strong>□</strong> 이 판정이 한국 2017~2025·12개월 지평에서 확인됐을 뿐 미국 같은 기간에서는 확인되지 않았다는 한계와 데이터 기준일을 봤다.<br>
