...
5. **자본비용 → 기업·뉴스**에서 반대 근거를 찾습니다. 사후검증(백테스트) 탭은 **무기한 보류**했습니다 — 판정과 다른 계산을 검증하고 있었고 무료 데이터 길이로는 정렬이 불가능합니다. 자기검증은 요약 탭의 가중치 민감도·방법 간 편차·컨센서스 병기가 대신합니다.
"""
    )
    st.info(
...
<div class="guide-check">
  ...
  <strong>□</strong> 이 도구가 자기 판정을 사후검증하지 못한다는 한계와 데이터 기준일을 봤다.<br>
