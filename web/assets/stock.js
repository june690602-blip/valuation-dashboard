...
(function () {
  ...
  function renderHeader() {
    ...
    /* 가중치 폭 (HANDOFF-AXES 4단계 · ADR-0041) — **가중치 숫자는 바꾸지 않고 폭만 보여준다.**
       문헌(LNT 2002/2007)은 순위까지만 정하므로, 그 순위를 지키는 가중치들이 만드는 폭이
       곧 **문헌이 실제로 못 정한 만큼**이다.
       전 종목 공통 상수(±N%)를 적지 않는다. 패널에서 이 폭의 중앙값이 5.65%인데 90분위는
       11.52%로 **꼬리만 크게 움직여서**, 한 숫자를 박으면 대부분 종목에서 과장이 되고
       일부에서는 과소가 된다. 파이썬이 이 종목의 실제 양 끝을 계산해 보내고 여기서는
       문장만 만든다.
       ⚠ 인계문이 쓰라던 두 탐침(동일가중·③가중 2배)은 재정규화하면 순위를 어긴다 —
       ③가중 2배는 ③을 최상위로 역전시킨다. 그래서 그 값이 아니라 이 범위를 쓴다. */
    var wr = v.weight_range, widthBody = '';
    if (wr && wr.low != null && wr.high != null) {
      widthBody = '<b>가중치를 달리 두면 얼마나 움직이는가</b> — 문헌은 순위(' +
        esc(wr.top_marks) + ' &gt; ' + esc(wr.lower_marks) + ')까지만 정하고 숫자는 정하지 ' +
        '않습니다. 그 순위를 지키는 범위에서 ' + esc(wr.lower_marks) + '의 몫을 0부터 ' +
        esc(wr.top_marks) + '와 같아질 때까지 움직이면 적정가는 <b class="mono">' +
        fmtPrice(wr.low) + '</b>–<b class="mono">' + fmtPrice(wr.high) + '</b>' +
        '(지금 표시된 값 대비 ' + fmtSigned(wr.rel_low) + ' ~ ' + fmtSigned(wr.rel_high) + ')' +
        '에서 움직입니다' +
        // 판정 어휘를 조사 앞에 두지 않는다 — '고평가과/수준과'처럼 받침에 따라 갈리는데
        // 이 코드베이스에 조사 헬퍼가 없다. 끝점을 나열하는 형태가 문법도 안 틀리고
        // **어느 끝이 어느 판정인지**까지 말해 준다.
        (wr.flips
          ? ' — <b>그 안에서 판정이 갈립니다</b>: 아래 끝 <b>' + esc(wr.verdict_low) +
            '</b> · 위 끝 <b>' + esc(wr.verdict_high) + '</b>.'
          : ' — 판정은 그 안에서 바뀌지 않습니다.');
    }
    /* 근거 셋(무엇에 기대는가 · 얼마나 틀리는가 · 가중치를 달리 두면)을 접힘 하나로 묶는다.
       ...
       상자들을 각각 접지 않고 하나로 묶는 이유: 접힘이 셋씩 줄서면 접기 전과 같은
  ...
  function renderSummary() {
    ...
    var rows = order.map(function (name) {
      ...
      if (e) {
        ...
        // 값은 나왔는데 판정 가중이 없는 방법이 있다 — 빈칸으로 두면 "빠뜨렸나"로 읽히니
    ...
    // 아니라 결론이 얼마나 흔들리는지의 근거고, 4단계의 가중치 폭이 여기 얹혔다(ADR-0041).
    ...
    var formula = fold('계산식과 출처 · 민감도 · ④를 판정에서 뺀 이유',
      '<b>계산식</b><br/>' +
      '① 업종·규모·수익성 회귀로 구한 적정 배수(PER·PBR·EV/EBITDA) × 자사 펀더멘털 — 계수가 없으면 규모 비교가능 피어 중앙값으로 폴백합니다 &nbsp;② 자기 과거 PER·PBR 25~75분위 × 현재 EPS·BPS &nbsp;③ RIM: V = B + B(ROE−r)·w/(1+r−w), r = CAPM 자기자본비용 &nbsp;④ 컨센서스 12개월 EPS × 자기 과거 PER 중앙값 &nbsp;⑤ 최근 8년(짧으면 있는 만큼 · 최소 3년) 평균 순이익 × ①의 회귀 적정 PER<br/>' +
      ...
      '<b>③의 지속계수 w 근거</b> · 초과이익이 매년 얼마나 남는지를 정하는 값이고 ③의 감도를 사실상 혼자 결정합니다. 세 시나리오 모두 문헌값입니다 — 0.21 Wang 2023(EAR)·Myers 1999(TAR), 중심 0.62 Fama·French 2000(JB, 수익성 평균회귀 연 38%), 1.00 Frankel·Lee 1998(JAE, 잔여이익 영구 지속). 전부 미국 표본이라 한국에 그대로 성립한다고 보증하지 못합니다. 상세 · docs/adr/0039<br/>' +
      sens +
      bookQualityLine(v) +
  ...
  function renderBacktest() {
    ...
    if (vg != null && ld != null) {
      ...
      lede += '<div class="cons-callout' + (flip ? ' clash' : '') + '">' +
        ...
        (flip ? ' — <b>방향이 반대입니다.</b>' : '.') +
        ' 두 값은 재료가 달라 어긋날 수 있어요: 판정에는 <b>① 업종 상대가치</b>가 들어가지만 여기엔 없고(이 패널은 ②③만 복원합니다), ' +
    ...
    var btScope = fold('이 관찰의 범위와 한계 · 왜 승률·전략수익을 안 보여주나',
      ...
      (methodNote ? '<b>어떤 신호를 복원했나</b> · ' + methodNote + ' ① 업종 상대가치는 이 패널이 복원하지 않습니다 — 회귀 적정 배수(ADR-0014)에 필요한 것은 피어 <b>목록</b>이 아니라 그 시점의 <b>횡단면</b>인데, 단일 종목을 보는 이 틀은 그것을 만들지 않습니다. ④ 선행 이익은 과거 <b>시점별</b> 컨센서스 EPS 빈티지가 무료 데이터에 없습니다<br/>' : '') +
      ...
      '<b>통계적으로 옳은 길</b> · 여러 종목 × 여러 시점의 <b>횡단면 검증</b>이고, 그것은 이 패널 밖에서 <b>실제로 했습니다</b> — 한국 1,292종목(폐지 332곳 포함) · 9개 시점(2017~2025)으로 판정을 채점했습니다(ADR-0028). 시점별 유니버스는 KRX-DELISTING과 FDR로 복원됩니다. 다만 <b>미국 같은 기간에서는 확인되지 않았습니다</b>(ADR-0030). 이 패널은 단일 종목이라 그 자리를 대신하지 못해, 통계 대신 관찰로 층위를 내렸습니다');
