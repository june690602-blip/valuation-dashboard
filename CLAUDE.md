...
> 그 절 머리에 **"시작하기 전에 — 3단계에서 알게 된 것"** 여덟 항목이 있습니다.
> **모르면 사고 나는 것들이라 먼저 읽으세요.**
>
> **가중치 숫자는 바꾸지 않습니다.** 문헌이 순위까지만 정하므로 그 순위를 지키는
> 가중치들이 만드는 **폭만** 1단계의 접힘 안에 적는 일입니다.
>
> **⚠ 그 폭으로 적혀 있던 ±6%는 죽은 숫자입니다.** 판정이 4축(①②③⑤)이던 시절 값인데
> ②를 뺐으므로 지금은 3축입니다. 다시 재면 **동일가중 10.9% · ③가중 2배 15.3%**로
> **거의 두 배**입니다. **그대로 화면에 쓰면 거짓말이 됩니다.**
> 게다가 어느 통계로 말하느냐에 따라 또 두 배 갈립니다(부호 있는 90분위 10.9% 대
> 절대값 90분위 23.8%) — **무엇의 N인지 먼저 정하고 그 다음에 재세요.**
>
> **⚠ 재기 전에 `data/backtest/panel.parquet`을 다시 만들어야 합니다** —
> [ADR-0040](docs/adr/0040-close-the-open-top-roe-bucket.md)이 ①⑤의 배수를 바꿔
> **스테일**해졌습니다(`code_commit` ≠ HEAD). 위 10.9%/15.3%도 그 스테일한 패널에서 낸
> 값이라 **방향만 믿고 숫자는 다시 재세요.** `backtest_panel.py --market KR`(14분) →
> `backtest_pack.py pack`. `raw/`는 이 PC에 있습니다.
>
...
> **다음 번호는 0041**인데 **0039가 아직 main에 없습니다**(rim-persistence가 다른 브랜치에
> 떠 있습니다). `check_adr_index.py`는 **브랜치를 가로질러 보지 못합니다** — 두 방이 같은
> 번호를 쓰고 문구만 다르면 중복으로 안 걸리고 그대로 머지됩니다. 실제로 0038이 그럴 뻔했습니다.
>
