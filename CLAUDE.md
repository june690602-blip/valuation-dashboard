...
> 브랜치 `fix/peer-selection-by-size`에서 ①(업종 상대가치)을 피어 중앙값에서
> **업종·규모·수익성 회귀**로 바꿨습니다(ADR-0014). **11개 태스크 전부 끝났고 아직 푸시 안 했습니다.**
> 다음은 **ADR-0013**입니다 — 다만 목적이 "①을 살린다"에서 "업종 백분위 점수의 규모 편향을
> 없앤다"로 바뀌었으니 문서부터 다시 쓰세요(이유는 HANDOFF).
