/* stock.js를 손댈 때 **화면이 한 글자도 안 바뀌었는지** 재는 도구.

   `stock.js`는 2,400줄이 IIFE 하나에 살아서 밖에서 부를 손잡이가 없다 — 그래서 이 파일을
   리팩터할 때마다 "동작이 그대로인가"를 증명할 방법을 매번 새로 지어야 했다(#80·#81에서
   두 번 그랬다). 그 일회용을 없애려고 남긴다.

   ⚠ **분할 전용 도구가 아니다.** 파일을 쪼개든, 제자리에서 중복을 모으든, 전역의 주인을
   바꾸든 — `stock.js`의 출력이 안 바뀌어야 하는 모든 변경에 쓴다.
   (줄 수를 줄이는 것은 이 저장소의 목표가 아니다 — `check_structure.py` 머리말이
   *"줄 수는 읽기 어려움의 대리 지표일 뿐"*이라며 크기 상한 관문을 이미 없앴다.)
...
/* ── 고정 입력 ──────────────────────────────────────────────────────
   ⚠ 손대는 경로의 입력을 **먼저 여기 추가하고** 재라 — 여기 없는 분기는
   통과해도 아무것도 증명하지 않는다. */
