...
범위를 네 가지 방법으로 추정하고, 현재가가 그 범위 어디에 있는지 — 그리고 **왜 그렇게
...
| ② 요약·판정 | **결론** | 펀더멘털 적정가(①②③ 가중 종합) vs 현재가 — 컨센서스 반영값(④ 포함)을 바로 옆에 병기, 방법별 가중·재정규화 공개, 신뢰도(방법 간 편차) 툴팁, 업종 상대점수, 규칙 기반 판정 근거 |
...
| ~~백테스트~~ | **무기한 보류** | 판정(①②③)과 **다른 계산**을 검증하고 있었습니다 — ①은 아예 없고, ②의 기준배수도 판정은 자기 5년 분위인데 백테스트는 직전 1.5년 롤링 중앙값입니다(삼성전자 16.04배 vs 25.08배, 56% 차이). 입구만 닫았고 코드는 그대로입니다(ADR-0009) |
...
- **적정주가 — 판정은 펀더멘털 3방법, 컨센서스는 병기**(ADR-0006):
  판정 = ① 업종 상대가치(피어 중앙값 멀티플) ② 역사적 밴드(자기 5년 25~75분위)
  ③ 수익가치 RIM 의 가중평균(①38.5·②38.5·③23.1%). 셋 다 회사가 **이미 낸** 실적·자산에서
  나온 값이라 시장 기대와 독립적으로 계산됩니다.
  ...
  실제 적용 가중을 표에 공개합니다.
- **판정**: 펀더멘털 종합 괴리율 5단계(±10%/±30%) + 방법 간 편차(변동계수)로 신뢰도 산출 — 근거를 툴팁으로 공개.
...
- **이 도구는 자기 판정을 사후검증하지 못합니다**(ADR-0009). 백테스트를 만들어 봤지만 판정과
  **다른 계산**을 검증하고 있었고(①부재 + ②의 창이 5년 vs 1.5년), 창을 맞추려면 테스트 시작
  전에 5년치가 더 필요한데 무료 주가 데이터가 총 5년뿐이라 정렬이 불가능합니다. 탭은 보류하고
  자기검증은 **가중치 민감도·방법 간 편차(신뢰도)·컨센서스 병기**로 대신합니다.
